...
namespace EnterpriseWorkflow.Elsa.RuleEngineAdapter
{
    ...
    public class EvaluateRuleActivity : Activity
    {
        ...
        [Input] public Input<string?>? RuleNames { get; set; }
        ...
        protected override async ValueTask ExecuteAsync(ActivityExecutionContext context)
        {
            ...
            // NEW: parse optional rule names from the input
            var ruleNamesRaw = context.Get(RuleNames);
            var ruleNames = string.IsNullOrWhiteSpace(ruleNamesRaw)
                ? null
                : ruleNamesRaw
                    .Split(',', StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries);

            if (ruleNames is { Length: > 0 })
            {
                logger.LogInformation("[EvaluateRule] Executing specific rules: [{Rules}]",
                    string.Join(", ", ruleNames));
            }

            // Pass rule names — null means execute all (original behaviour preserved)
            var results = await ruleEngine.ExecuteWorkflowAsync(workflowName, input, ruleNames);
